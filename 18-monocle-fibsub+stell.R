...
fibsub = subset(x = sce.har.db, idents = c("Fibroblast", "Stellate"))
table(fibsub@meta.data$celltype)
...
save(cds, col_vector, file = "./16-monocle2/fibsub.monocle.Rdata")
...
library(monocle)
library(Seurat)
load(file = "./16-monocle2/fibsub.monocle.Rdata")
...
ggsave(filename = "./16-monocle2/1-qc.pdf", width = 5, height = 3.5)
...
## 保存数据
save(cds, file = "./16-monocle2/monocle.input.cds.Rdata")


# 运行monocle
rm(list = ls()) 
load(file = "./16-monocle2/monocle.input.cds.Rdata")
cds 

# 接下来很重要，到底是看哪个性状的轨迹
table(pData(cds)$celltype)
pData(cds)$seurat_clusters = factor(pData(cds)$celltype, levels = c("Stellate","Fibroblast"))
...
cds$State = factor(cds$seurat_clusters, levels = c("Stellate", "Fibroblast"))
cds <- orderCells(cds, root_state = "Stellate")

# 最后两个可视化函数，对结果进行可视化
plot_cell_trajectory(cds, color_by = "seurat_clusters", size=1, show_backbone=TRUE)
ggsave(filename = "./16-monocle2/2-monocle_cell_trajectory_for_seurat.pdf", width = 4.8, height = 3.5)
length(cg)
plot_genes_in_pseudotime(cds[cg,], color_by = "seurat_clusters") 
ggsave(filename = "./16-monocle2/3-monocle_plot_genes_in_pseudotime_for_seurat.pdf", width = 5.5, height = 5)
...
save(cds, my_pseudotime_de, file = "./16-monocle2/output_of_monocle.Rdata")
...
library(dplyr)
library(ggsci)
load(file = "./16-monocle2/output_of_monocle.Rdata")
load(file = "./0-Rawdata/my_color.Rdata")
phe = pData(cds)
...
ggsave(filename = "./16-monocle2/4-c0-genes-pseudotime.pdf", width = 5.06, height = 5.19)
...
ggsave(filename = "./16-monocle2/5-c0-genes-pseudotime.pdf", width = 5.6, height = 4)
...
plot_genes_in_pseudotime(cds[rownames(cds) %in% my_pseudotime_gene,], color_by = "Cluster") + scale_color_manual(values = col_vector)
ggsave(filename = "./16-monocle2/6-c1-genes-pseudotime.pdf", width = 5.06, height = 5.19)

plot_genes_jitter(cds[rownames(cds) %in% my_pseudotime_gene,], grouping = "Cluster", color_by = "Cluster", ncol = NULL ) + scale_color_manual(values = col_vector)
ggsave(filename = "./16-monocle2/7-c1-genes-pseudotime.pdf", width = 5.6, height = 4)
...
ggsave(filename = "./16-monocle2/14-P1-P5-cluster.pdf", width = 9, height = 4.5)


# 亚群和拟时间
plot_cell_trajectory(cds, color_by = "seurat_clusters", size=1, show_backbone=TRUE) + scale_color_aaas()
ggsave(filename = "./16-monocle2/9-monocle_cell_trajectory_for_seurat.pdf", width = 4.8, height = 3.5)

plot_cell_trajectory(cds, color_by = "Pseudotime", size=1, show_backbone=TRUE) + scale_color_continuous(type = "viridis")
ggsave(filename = "./16-monocle2/10-monocle_cell_trajectory_for_Pseudotime.pdf", width = 4.8, height = 3.5)

# 单个基因
plot_cell_trajectory(cds, markers = "C7", use_color_gradient = T)
ggsave(filename = "./16-monocle2/11-monocle_cell_trajectory_for_C7.pdf", width = 4.8, height = 3.5)

plot_cell_trajectory(cds, markers = "MMP11", use_color_gradient = T)
ggsave(filename = "./16-monocle2/12-monocle_cell_trajectory_for_MMP11.pdf", width = 4.8, height = 3.5)

plot_cell_trajectory(cds, markers = "COL11A1", use_color_gradient = T)
ggsave(filename = "./16-monocle2/13-monocle_cell_trajectory_for_COL11A1.pdf", width = 4.8, height = 3.5)
